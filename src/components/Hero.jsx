...
export default function Hero({ data, regionId, onRegionChange }) {
  const quickStats = [
    { value: fmtInt(data.ejecutadasTotal.cantidad), label: 'Intervenciones ejecutadas' },
    { value: fmtInt(data.ejecutadasTotal.poblacion), label: 'Población beneficiada' },
    { value: data.flotaTotal, label: 'Unidades operativas' },
  ]
    ...
    <section id="top" className="relative flex min-h-[100svh] items-center overflow-hidden bg-surface-0">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 pt-28 pb-16">
        ...
        >
          ...
          <RegionSwitcher regionId={regionId} onChange={onRegionChange} />
        </motion.div>
        ...
        >
          Región <span className="text-brand-soft">{data.shortLabel}</span>
        </motion.h1>
      ...
      <motion.a
        href="#panorama"
        aria-label="Ir al panorama general"
        initial={{ opacity: 0 }}
