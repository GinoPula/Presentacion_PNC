...
(async () => {
  const browser = await chromium.launch({
    ...
    args: [
      '--proxy-server=http://127.0.0.1:33133',
  ...
  async function switchRegion(label) {
    const switcherBtn = await page.locator('button', { hasText: /Tumbes|Puno|Tacna|Piura|Ancash|Lambayeque|Ica/ }).first();
  ...
  const regions = ['Región Tumbes', 'Región Puno', 'Región Tacna', 'Región Piura', 'Región Ancash', 'Región Lambayeque', 'Región Ica'];
  for (const label of regions) {
    const slug = label.split(' ')[1].toLowerCase();
    await switchRegion(label);
    await page.waitForTimeout(1000);
