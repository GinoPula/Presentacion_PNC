  ...
  return `
    <div class="mapa-popup">
      ...
      <div class="mapa-popup-body">
        ...
        <div class="mapa-popup-row"><span>Ubicación</span><b>${p.provincia}${ubic ? ' · ' + ubic : ''}</b></div>
...
export default function MapaIntervenciones({ regionId, shortLabel }) {
  ...
  const allPoints = mapaIntervenciones[regionId] || []
  const limites = mapaLimites[regionId] || null
  ...
  // Puntos dentro del filtro territorial (provincia/distrito), sin importar las capas activas —
  // se usa para encuadrar el mapa y para los contadores "En esta vista".
  const scopedPoints = useMemo(
    () => allPoints.filter((p) => (!provincia || p.provincia === provincia) && (!distrito || p.distrito === distrito)),
    [allPoints, provincia, distrito],
  )
  ...
  // Si al cambiar de capa la provincia/distrito elegidos dejan de tener intervenciones del estado
  // activo, se limpian solos para no dejar el filtro apuntando a una selección vacía.
  useEffect(() => {
  ...
  // Vuela con zoom animado y margen hacia la provincia/distrito elegido (no reacciona al solo activar/desactivar capas).
  useEffect(() => {
    const map = mapRef.current
    if (!map) return
    const key = `${provincia}|${distrito}`
    ...
    flyToScope(map, allPoints, scopedPoints, distrito)
  ...
  const scopeLabel = provincia ? `${distrito ? distrito + ', ' : ''}${provincia}` : 'Vista regional completa'

  return (
    <section id="mapa" className="relative border-t border-white/[0.05] bg-surface-0 py-24 sm:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeading
          eyebrow="Georreferenciado"
          title="Mapa de intervenciones"
          description={`Ubicación de las intervenciones ejecutadas y en ejecución en ${shortLabel}, con ficha técnica, fechas y avance de cada punto. Activa o desactiva capas y filtra por provincia y distrito.`}
        />
